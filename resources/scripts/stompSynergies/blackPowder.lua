...
local PowderID = CollectibleType.COLLECTIBLE_BLACK_POWDER

---@param player EntityPlayer
function mod:BlackPowderStomp(player)
	if funcs.DefensiveStomp(player) then return end
	if not player:HasCollectible(PowderID) then return end	

	local randomSpawn = player:GetCollectibleRNG(PowderID):RandomInt(1, 3)
